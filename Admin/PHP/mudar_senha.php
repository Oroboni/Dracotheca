...
$sql = "SELECT SEN_BIBLIOT FROM bibliotecaria WHERE ID_BIBLIOT = ?";
...
if ($result && mysqli_num_rows($result) === 1) {
    ...
    if (password_verify($senha_atual, $row['SEN_BIBLIOT'])) {
        $senha_nova_hash = password_hash($senha_nova, PASSWORD_DEFAULT);

        $update_sql = "UPDATE bibliotecaria SET SEN_BIBLIOT = ? WHERE ID_BIBLIOT = ?";
